...
def find_runs(workspace: Path) -> list[dict]:
    ...
    # build_run() always sets the "eval_id" key (possibly to None), so
    # r.get("eval_id", float("inf")) never falls back to the default —
    # None must be normalized explicitly or sort() raises comparing it to int.
...
def generate_html(
    ...
    # Escape "<" so embedded content (arbitrary eval output/prompt text) can
    # never contain a literal "</script>" that would close the script block
