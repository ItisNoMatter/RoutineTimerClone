...
このPRがどのissueに対応するかを特定する。GitHubは `close #N` のようなキーワードがPR本文にあり、かつそのPRが**マージされた時点で**該当issueを自動closeする機能を持つが、キーワードを書かなければ発火しないため、ここで明示的に特定する。

- まずブランチ名・コミットメッセージに issue番号らしき記述(`#\d+`)が無いか確認する。
- 見つからなければ、`gh issue list --search "<diffやコミットログから拾ったキーワード>" --state open --limit 10` で候補を探し、`gh-issue-creator` スキルの親issue検索と同じ流儀で「このissueに対応するPRですか?」とユーザーに確認する。番号を推測で決めつけない。
- 対応するissueが無い(chore的な変更、issue化されていない作業など)と分かれば、その旨を明記して次に進んでよい。
...
- `{closes}`: ステップ3で特定した関連issueの扱いを書く。
  - このPRが該当issueのclose条件を**完全に**満たす場合のみ、`close #<番号>` の形式で1行ずつ記載する(複数issueを閉じる場合は複数行)。表記は必ず小文字の `close #` を使う(このリポジトリで実績のある形式)。
  - close条件が「エミュレータ確認」「スクショ添付」等の手動確認を要求しており、まだ実施していない場合はcloseキーワードを使わず `関連: #<番号>(close条件は手動確認が必要なため別途確認)` と書く。実態と異なる自動closeを起こさないこと。
  - 対応するissueが無ければ `なし`。
- `{notes}`: レビュー時の注意点など。特になければ「特になし」
...
- GitHubがissue自動closeキーワードとして認識するのは `close`/`closes`/`fix`/`fixes`/`resolve`/`resolves` + `#N`(大文字小文字不問)。このリポジトリでは `close #N` に統一する。issueへの単なる言及(「関連: #N」等)はこれらのキーワードを含まないため自動closeされない — その違いを理解した上で{closes}を書き分けること。
